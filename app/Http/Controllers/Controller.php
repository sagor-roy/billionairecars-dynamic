...
abstract class Controller extends BaseController
{
	...
	function validatePost($request)
	{
		...
		foreach ($str as $f) {
			...
			if ($field == 'created_at') $data['created_at'] = date('Y-m-d H:i:s');
			if ($field == 'updated_at') $data['updated_at'] = date('Y-m-d H:i:s');
			if ($f['view'] == 1) {
					...
					if ($f['type'] == 'file') {
						...
						if ($f['option']['upload_type'] == 'file') {
							...
							if (isset($f['option']['image_multiple']) && $f['option']['image_multiple'] == 1) {
								...
								$destinationPath = '.' . $f['option']['path_to_upload'];
								//dd($request->file($f['field']));
								if ($request->hasFile($f['field']) && (count($request->file($f['field'])) >= 1 || $request->file($f['field']) == null)) {
