...
class UserController extends Controller
{
	...
	public function reCaptcha($request)
	{
		if (!is_null($request['g-recaptcha-response'])) {
			$post_data             = array();
			$post_data['secret']   = config('sximo.cnf_recaptchaprivatekey');
			$post_data['response'] = $request['g-recaptcha-response'];
	...
	public function postSignin(Request $request)
	{
		...
		if (config('sximo.cnf_recaptcha') == 'true') {
			...
			if ($return === false || !$return['success'])
				return response()->json(['status' => 'error', 'message' => 'Invalid reCpatcha']);
		}
		$validator = Validator::make($request->all(), $rules);
		if ($validator->passes()) {
			...
			if (
				\Auth::attempt(array('email' => $request->input('email'), 'password' => $request->input('password')), $remember)
				or
	...
	{
		if (in_array($social, array('facebook', 'google')))
			$user = Socialize::driver($social)->user();
		else
			return redirect('user/login')
				->with(['message' => 'Invalid login attempt', 'status' => 'error']);


		if (empty($user->email))
			return redirect('user/login')
				->with(['message' => 'Unable to get your mail address ', 'status' => 'error']);

		$authen = User::where('email', $user->email)->first();

		if ($authen) {

			if (empty($authen->avatar) || !file_exists(public_path('uploads/users/' . $authen->avatar))) {
				$filename = uniqid() . '.jpg';
				copy($user->avatar, public_path('uploads/users/' . $filename));
				$authen->avatar = $filename;
				$authen->save();
			}

			return self::autoSignin($authen->id);
		} else {
			...
			$authen 			  = new User;
			$authen->username     = $user->email;
			$authen->first_name   = $name_arr[0];
			$authen->last_name    = trim(str_replace($name_arr[0], "", $user->name));
			$authen->email 		  = strtolower($user->email);
			$authen->group_id 	  = $this->config['cnf_group'];
			$authen->avatar 	  = $filename;
			$authen->password 	  = \Hash::make(uniqid());
			$authen->active		  = 1;
