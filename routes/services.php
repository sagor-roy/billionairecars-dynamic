<?php
        
// Start Routes for adminproduct 
Route::resource('services/adminproduct','Services\AdminproductController');
// End Routes for adminproduct 

                    
// Start Routes for adminbrand 
Route::resource('services/adminbrand','Services\AdminbrandController');
// End Routes for adminbrand 

                    
// Start Routes for adminfaqs 
Route::resource('services/adminfaqs','Services\AdminfaqsController');
// End Routes for adminfaqs 

                    
// Start Routes for admincontact 
Route::resource('services/admincontact','Services\AdmincontactController');
// End Routes for admincontact 

                    
// Start Routes for adminhomeslider 
Route::resource('services/adminhomeslider','Services\AdminhomesliderController');
// End Routes for adminhomeslider 

                    
// Start Routes for admincarplan 
Route::resource('services/admincarplan','Services\AdmincarplanController');
// End Routes for admincarplan 

                    ?>