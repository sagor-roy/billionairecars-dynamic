...
@section('content')
    ...
    @include('sximo.module.tab', ['active' => 'table', 'type' => $type])
    <div class="p-3">
        {!! Form::open([
            'url' => 'sximo/module/savetable/' . $module_name,
        ...
        <div class="infobox infobox-success">
            <p> <strong>New Feature ! ( LIMIT TO ) </strong> Type User ID's using (,) into spesific column to limit the
                column only viewd by them </p>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="text" class="form-control form-control-sm" width="40"
                                name="limited[<?php echo $id; ?>]" class="limited" value="<?php echo $limited_to; ?>"
                                style="width: 30px" />
                        ...
                        <td>
                            <div class="input-group input-group-sm">
                                ...
                                <input name="label[<?php echo $id; ?>]" type="text"
                                    class="form-control form-control-sm " id="label" value="<?php echo $rows['label']; ?>" />
                            </div>
				   ...
				   ?>
                                <div class="input-group input-group-sm" style="margin:1px 0 !important;">
                                    <div class="input-group-prepend"><span
                                            class="input-group-text"><?php echo strtoupper($l['folder']); ?></span></div>
                                    <input name="language[<?php echo $id; ?>][<?php echo $l['folder']; ?>]" type="text"
                                        class="form-control form-control-sm " value="<?php echo isset($rows['language'][$l['folder']]) ? $rows['language'][$l['folder']] : ''; ?>"
                        ...
                        <td>
                            <label>
                                <input name="view[<?php echo $id; ?>]" type="checkbox" id="view" value="1"
                                    class="minimal-green" <?php if ($rows['view'] == 1) {
                        ...
                        <td>
                            <label>
                                <input name="detail[<?php echo $id; ?>]" type="checkbox" id="detail" value="1"
                                    class="minimal-green" <?php if ($rows['detail'] == 1) {
                                        echo 'checked="checked"';
                                    } ?> />
                            </label>
                        </td>
                        <td>
                            <label>
                                <input name="sortable[<?php echo $id; ?>]" type="checkbox" id="sortable" value="1"
                                    class="minimal-green" <?php if ($rows['sortable'] == 1) {
                                        echo 'checked="checked"';
                                    } ?> />
                            </label>
                        </td>
                        <td>
                            <label>
                                <input name="download[<?php echo $id; ?>]" type="checkbox" id="download" value="1"
                                    class="minimal-green" <?php if ($rows['download'] == 1) {
                                        echo 'checked="checked"';
                                    } ?> />
                            </label>
                        </td>
                        <td>
                            <input type="text" class="form-control form-control-sm" name="width[<?php echo $id; ?>]"
                                value="<?php echo $rows['width']; ?>" />
                        </td>
                        <td>
                        ...
                        <td>
                            <select class="form-control form-control-sm" name="format_as[<?php echo $id; ?>]"
                                style="width:100px;">
                                <option value=''> None </option>
                                @foreach ($formats as $key => $val)
                                    <option value="{{ $key }}" <?php if (isset($rows['format_as']) && $rows['format_as'] == $key) {
                                        echo 'selected';
                                    } ?>> {{ $val }} </option>
                                @endforeach
                            </select>

                            <input type="text" name="format_value[<?php echo $id; ?>]" value="<?php if (isset($rows['format_value'])) {
                                echo $rows['format_value'];
                            } ?>"
                                class="form-control form-control-sm" style="width:auto !important; display:inline;">
                            ...
                            <input type="hidden" name="hidden[<?php echo $id; ?>]" value="<?php if (isset($rows['hidden'])) {
                                echo $rows['hidden'];
                            } ?>" />
                            ...
                            <input type="hidden" name="conn_valid[<?php echo $id; ?>]" value="<?php if (isset($rows['conn']['valid'])) {
                                echo $rows['conn']['valid'];
                            } ?>" />
                            <input type="hidden" name="conn_db[<?php echo $id; ?>]" value="<?php if (isset($rows['conn']['db'])) {
                                echo $rows['conn']['db'];
                            } ?>" />
                            <input type="hidden" name="conn_key[<?php echo $id; ?>]" value="<?php if (isset($rows['conn']['key'])) {
                                echo $rows['conn']['key'];
                            } ?>" />
