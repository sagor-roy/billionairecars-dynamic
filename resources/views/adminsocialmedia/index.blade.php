...
<div class="toolbar-nav" >   
	<div class="row">
		<div class="col-md-8"> 	
			@if($access['is_add'] ==1)
			<a href="{{ url('adminsocialmedia/create?return='.$return) }}" class="btn  btn-sm"  
...
<div class="table-container">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'adminsocialmedia?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		    ...
		    <table class="table  table-hover " id="{{ $pageModule }}Table">
		        <thead>
					<tr>
						...
						@if($access['is_detail'] ==1 || $access['is_edit'] ==1 || $access['is_remove'] ==1 )
							...
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('adminsocialmedia/'.$row->id.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('adminsocialmedia/'.$row->id.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
